...
plot(model1$fitted.values, datnf$agn, xlab = "fitted values", ylab = "agn", main = "Прогноз - відгук, модель 1")
abline(0,1, col = "red")
plot(model1$fitted.values, model1$residuals, xlab = "fitted values", ylab = "residials", main = "Прозноз-залишки, модель 1")
library(carData)
library(car)
influencePlot(model1)
model1<-lm(agn~.-agn, data = datnf)
model2<-lm(agn~.-agn, data = datnf[-(c(2335,2358,2360,2361,2374)-2334+1),])
summary(model2)
plot(model2$fitted.values, datnf[-(c(2335,2358,2360,2361,2374)-2334+1),1], xlab = "fitted values", ylab = "agn", main = "Прогноз - відгук, модель 2")
abline(0,1, col = "red")
plot(model2$fitted.values, model2$residuals, xlab = "fitted values", ylab = "residials", main = "Прозноз-залишки, модель 2")
...
points(1:length(model2$residuals), model2$residuals, pch = 19, col = "red")
...
modelf2<-lm(agn~.-agn,data=Data[-(c(2335,2358,2360,2361,2374)-2334+1),][(nn-200):(nn-10),])
summary(modelf2)
